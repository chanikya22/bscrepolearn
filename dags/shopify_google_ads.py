...
# Import task bae functions
from connections.bsc.shopify.google_ads.google_bae.extract import extract_data  as extract_data_bae
from connections.bsc.shopify.google_ads.google_bae.load import load_data as load_data_bae

# Import task bsc functions
from connections.bsc.shopify.google_ads.google_bsc.extract import extract_data as extract_data_bsc
from connections.bsc.shopify.google_ads.google_bsc.load import load_data as load_data_bsc
...
# Create DAG directly
dag = DAG(
    dag_id='shopify_google_ads_pipeline',
    description='ETL pipeline for Google Ads data extraction and loading for BAE',
    schedule_interval='0 0,12 * * *',  # Daily at 5:30 AM IST (00:00 UTC) and 5:30 PM IST (12:00 UTC)
    ...
    tags=['etl', 'bae', 'google-ads', 'marketing', 'api'],
)
start = EmptyOperator(
    task_id='start',
    dag=dag,
...
bae_extract_task = TrackedPythonOperator(
    task_id='bae_extract_google_ads_data',
    python_callable=create_extract_wrapper(extract_data_bae, 'google-bae-ads', 'google-bae-ads'),
    op_kwargs={
...
bae_load_task = TrackedPythonOperator(
    task_id='bae_load_google_ads_data',
    python_callable=load_data_bae,
    pipeline_name='google-bae-ads',
    client_id='bsc',
    data_type='google-bae-ads',
    is_last_task=True,
...
# Define the extract task
bsc_extract_task = TrackedPythonOperator(
    task_id='bsc_extract_google_ads_data',
    python_callable=create_extract_wrapper(extract_data_bsc, 'google-bsc-ads', 'google-bsc-ads'),
    op_kwargs={
        'report_types': ['campaigns', 'keywords', 'ads']  # Extract all report types else mention specific ones
    },
...
# Load task - loads the extracted data to database
bsc_load_task = TrackedPythonOperator(
    task_id='bsc_load_google_ads_data',
    python_callable=load_data_bsc,
    pipeline_name='google-bsc-ads',
    client_id='bsc',
    data_type='google-bsc-ads',
    is_last_task=True,
...
get_all_dates_task >> bae_extract_task >> bae_load_task >> bsc_extract_task >> bsc_load_task >> end_task