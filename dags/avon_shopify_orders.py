...
from connections._100days.avon.shopify.orders.extract import extract_data
from connections._100days.avon.shopify.orders.transform import transform_data_to_postgres
from connections._100days.avon.shopify.orders.load import load_data
...
def get_all_date_ranges(**context):
    ...
    """
    pipelines = [
        ('avon-shopify-orders', 'avon-shopify-orders')
    ]
...
dag = DAG(
    dag_id='avon_shopify_orders_pipeline',
    description='Avon Shopify Order ETL',
    schedule_interval='30 3 * * *',  # Daily at 9 AM IST
    default_args=default_args,
    start_date=datetime(2026, 2, 1),
    catchup=False,
    max_active_runs=1,
    tags=['etl', 'shopify', 'avon'],
)
...
extract = TrackedPythonOperator(
    task_id='avon_extract_from_source',
    python_callable=create_extract_wrapper(extract_data,"avon-shopify-orders"),
    pipeline_name='avon-shopify-orders',
    client_id='avon',
    data_type='avon-shopify-orders',
    is_first_task=True,
    is_last_task=False,
    dag=dag
)
...
transform = TrackedPythonOperator(
    task_id='avon_transform_from_raw',
...
def run_shopify_orders_consolidated():
    ...
    query = """
        SELECT * FROM avon.insert_shopify_consolidated();
    """
...
def run_shopify_sales_analysis():
    postgres = PostgresConnector(db_prefix="warehouse_")
    query = """
        CALL avon.sp_shopify_sales_analysis();
    """
    postgres.execute_query(query)
...
complete = EmptyOperator(task_id='avon_pipeline_complete',dag=dag)

# flow
start >> check_previous_run_task >> get_all_dates_task
get_all_dates_task >> extract >> transform >> load >>consolidated_task>>sales_analysis_task>> complete
