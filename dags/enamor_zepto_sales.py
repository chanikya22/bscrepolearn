...
from connections.enamor.zepto.sales.load import load_data
from connections.enamor.zepto.sales.extract import extract_data
...
# Create DAG directly
dag = DAG(
    dag_id='enamor_zepto_sales_connector',
    description='ETL pipeline for  Zepto Sales Connector for enamor',
    schedule_interval='30 9 * * *',  # Daily at 8:00 AM (IST)
    default_args=default_args,
    start_date=datetime(2025, 9, 11),
    catchup=False,
    tags=['el', 'enamor','zepto', 'Sales', 'connector'],
)
...
extract_task = TrackedPythonOperator(
    ...
    op_kwargs={
        ...
        'end_date': end_date,
        'file_upload_path': 'zepto/enamor/sales-report/',
    },
    pipeline_name='enamor_zepto_sales',
    client_id='enamor',
