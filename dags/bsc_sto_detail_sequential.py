...
from connections.bsc.vinculum.stodetail.extract import extract_data
from connections.bsc.vinculum.stodetail.transform import transform_data
from connections.bsc.vinculum.stodetail.load import load_data
...
def get_next_date_range(**context):
    ...
    """
    # Configuration
    START_DATE = datetime(2026, 6, 1)  # Your starting date
    END_DATE = datetime(2026, 6, 17)  # Your ending date
...
# Create DAG
dag = DAG(
    dag_id='vinculum_sto_detail_sequential',
    description='Sequential ETL pipeline for Vinculum STO Detail data (day by day)',
    schedule_interval=None,  # Manual trigger only
    default_args=default_args,
    start_date=datetime(2025, 3, 9),
    catchup=False,
    tags=['etl', 'vinculum', 'stodetail', 'sequential'],
)
...
trigger_next = TriggerDagRunOperator(
    ...
    trigger_dag_id='vinculum_sto_detail_sequential',  # Self-trigger
