...
from connections.enamor.blinkit.sales.load import load_data
from connections.enamor.blinkit.sales.extract import extract_data
...
# Create DAG directly
dag = DAG(
    dag_id='enamor_blinkit_sales_connector',
    description='ETL pipeline for  Blinkit Sales Connector for enamor',
    schedule_interval='30 9 * * *',  # Daily at 8:00 AM (IST)
    default_args=default_args,
    start_date=datetime(2025, 9, 11),
    catchup=False,
    tags=['el', 'enamor','blinkit', 'Sales', 'connector'],
)
...
extract_task = TrackedPythonOperator(
    ...
    op_kwargs={
        'brand': 'enamor',
        'file_upload_path': 'blinkit/enamor/sales-report/',
    },
    pipeline_name='enamor_blinkit_sales',
    client_id='enamor',
