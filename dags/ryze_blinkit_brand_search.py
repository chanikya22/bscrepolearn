...
# Create DAG directly
dag = DAG(
    dag_id='ryze_blinkit_brand_search',
    description='ETL pipeline for Blinkit Brand Search for ryze',
    schedule_interval='30 2 * * *',  # Daily at 8:00 AM (IST)
    default_args=default_args,
    start_date=datetime(2025, 5, 2),
    catchup=False,
    tags=['etl', 'ryze','blinkit', 'brandsearch', 'scraping'],
)
...
extract_task = TrackedPythonOperator(
    ...
    op_kwargs={
        ...
        's3_output_path': 'insytscraping/ryze/blinkit/brand_search/',
        'rate_limit': 500,
        'max_workers': None  # Will automatically set based on rate_limit
    },
    pipeline_name='ryze_blinkit_brand_search',
    client_id='ryze',
