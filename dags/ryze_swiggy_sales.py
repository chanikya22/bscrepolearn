...
from connections.ryze.swiggy.sales.load import load_data
from connections.ryze.swiggy.sales.extract import extract_data
...
# Create DAG directly
dag = DAG(
    dag_id='ryze_swiggy_sales_connector',
    description='ETL pipeline for  Swiggy Sales Connector for Ryze',
    schedule_interval='30 9 * * *',  # Daily at 8:00 AM (IST)
    default_args=default_args,
    start_date=datetime(2025, 8, 13),
    catchup=False,
    tags=['el', 'ryze','swiggy', 'Sales', 'connector'],
)
...
extract_task = TrackedPythonOperator(
    ...
    op_kwargs={
        ...
        'end_date':end_date,
        'file_upload_path': 'swiggy/ryze/sales-report/',
    },
    pipeline_name='ryze_swiggy_sales',
    client_id='ryze',
