...
def get_next_date_range(**context):
    ...
    """
    # Configuration
    START_DATE = datetime(2025, 12, 19)  # Your starting date
    END_DATE = datetime(2025, 12,23)  # Your ending date
...
def trigger_next_run(**context):
    ...
    try:
        ...
        if current_date >= datetime(2025, 12,23):
...
# Create DAG
dag = DAG(
    dag_id='vinculum_return_detail_sequential',
    description='Sequential ETL pipeline for Vinculum Return Detail data (day by day)',
    schedule_interval=None,  # Manual trigger only
    default_args=default_args,
    start_date=datetime(2025, 3, 9),
    catchup=False,
    tags=['etl', 'vinculum', 'returndetail', 'sequential', 'return'],
)
...
trigger_next = TriggerDagRunOperator(
    ...
    trigger_dag_id='vinculum_return_detail_sequential',  # Self-trigger
