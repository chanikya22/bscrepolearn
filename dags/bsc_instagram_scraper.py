...
# Instagram Post Details  Pipeline
from connections.bsc.instagram.details.posts.extract import extract_data as extract_posts
from connections.bsc.instagram.details.posts.transform import transform_data as transform_posts
from connections.bsc.instagram.details.posts.load import load_data as load_posts

# Instagram profile   Pipeline
from connections.bsc.instagram.profile.extract import extract_data as extract_profile
from connections.bsc.instagram.profile.transform import transform_data as transform_profile
from connections.bsc.instagram.profile.load import load_data as load_profile
...
def get_all_date_ranges(**context):
    ...
    """
    pipelines = [
        ('bsc-instagram-profile', 'bsc-instagram-profile'),
        ('bsc-instagram-detail-posts', 'bsc-instagram-detail-posts'),
...
dag = DAG(
    dag_id='bsc_instagram_scraper',
    description='ETL Pipeline for Instagram details/profile Apify Scraper',
    ...
    tags=['instagram', 'scraper', 'apify', 'bsc','socials'],
) 
start = EmptyOperator(
    task_id='start',
    dag=dag,
...
# # =============================================================================
# # LOAD INSTAGRAM POSTS SCRAPER PIPELINE
# # =============================================================================

# extract review tasks
bsc_extract_instagram_posts_data = TrackedPythonOperator(
    task_id='extract_instagram_posts_data',
    python_callable=create_extract_wrapper(extract_posts, 'bsc-instagram-detail-posts', 'bsc-instagram-detail-posts'),
    pipeline_name='bsc-instagram-detail-posts',
    client_id='bsc',
    data_type='bsc-instagram-detail-posts',
    is_first_task=True,
    trigger_rule='none_failed',  # Run if upstream didn't fail
    dag=dag
)

bsc_transform_instagram_posts_data = TrackedPythonOperator(
    task_id='bsc_transform_instagram_posts_data',
...
bsc_load_instagram_posts_data = TrackedPythonOperator(
    task_id='bsc_load_instagram_posts_data',
...
bsc_instagram_posts_complete = DummyOperator(
    task_id='bsc_instagram_posts_pipeline_complete',
...
# # =============================================================================
# # LOAD INSTAGRAM PROFILE SCRAPER PIPELINE
# # =============================================================================

# extract review tasks
bsc_extract_instagram_profile_data = TrackedPythonOperator(
    task_id='extract_instagram_profile_data',
    python_callable=create_extract_wrapper(extract_profile, 'bsc-instagram-profile', 'bsc-instagram-profile'),
    pipeline_name='bsc-instagram-profile',
    client_id='bsc',
    data_type='bsc-instagram-profile',
    is_first_task=True,
    trigger_rule='none_failed',  # Run if upstream didn't fail
    dag=dag
)

bsc_transform_instagram_profile_data = TrackedPythonOperator(
    task_id='bsc_transform_instagram_profile_data',
    python_callable=transform_profile,
    pipeline_name='bsc-instagram-profile',
    client_id='bsc',
    data_type='bsc-instagram-profile',
    is_last_task=False,
...
bsc_load_instagram_profile_data = TrackedPythonOperator(
    task_id='bsc_load_instagram_profile_data',
...
# review pipeline completion marker
bsc_instagram_profile_complete = DummyOperator(
    task_id='bsc_instagram_profile_pipeline_complete',
    trigger_rule='none_failed_min_one_success',
    dag=dag
)
...
# BSC Instagram Posts Pipeline (runs first)
get_all_dates_task >> bsc_extract_instagram_posts_data >> bsc_transform_instagram_posts_data >> bsc_load_instagram_posts_data >> bsc_instagram_posts_complete


# BSC Instagram Profile Pipeline (runs second)
bsc_instagram_posts_complete >> bsc_extract_instagram_profile_data >> bsc_transform_instagram_profile_data >> bsc_load_instagram_profile_data >> bsc_instagram_profile_complete >> end_task
