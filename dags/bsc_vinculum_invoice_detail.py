...
def get_next_date_range(**context):
    ...
    result = tracker.get_data_registry(
        client_id='bsc',
        data_type='vinculum-invoicedetail',
        is_current= 'true',
        limit=1,
        sort_by='run_id',
        sort_order='DESC'
...
# Create DAG directly
dag = DAG(
    dag_id='vinculum_invoice_detail',
    description='ETL pipeline for Vinculum Invoice Detail data',
    schedule_interval='15 * * * *',  # Every hour at minute 15
    default_args=default_args,
    start_date=datetime(2025, 3, 9),
    catchup=False,
    tags=['etl', 'vinculum', 'invoicedetail', 'invoice'],
)
