...
dag = DAG(
    dag_id='neurogum_shopify_orders_pipeline',
    description='Neurogum Shopify Order ETL',
    schedule_interval='30 3 * * *',  # Daily at 9 AM IST
    default_args=default_args,
    start_date=datetime(2026, 2, 1),
    catchup=False,
    max_active_runs=1,
    tags=['etl', 'shopify', 'neurogum'],
)
...
extract = TrackedPythonOperator(
    ...
    python_callable=create_extract_wrapper(extract_data,"neurogum-shopify-orders"),
...
complete = EmptyOperator(task_id='neurogum_pipeline_complete',dag=dag)

# flow
start >> check_previous_run_task >> get_all_dates_task
get_all_dates_task >> extract >> transform >> load >> complete
