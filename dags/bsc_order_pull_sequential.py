...
from connections.bsc.vinculum.orderpull.extract import extract_data
from connections.bsc.vinculum.orderpull.transform_v2 import transform_data
from connections.bsc.vinculum.orderpull.load_v2 import load_data
...
def get_next_date_range(**context):
    ...
    """
    # Configuration
    START_DATE = datetime(2025, 12,22)  # Your starting date
    END_DATE = datetime(2025, 12,24)  # Your ending date
...
def trigger_next_run(**context):
    ...
    try:
        ...
        if current_date >= datetime(2025, 12, 24):
...
# Create DAG
dag = DAG(
    dag_id='vinculum_order_pull_sequential',
    description='Sequential ETL pipeline for Vinculum Order Pull data (day by day)',
    schedule_interval=None,  # Manual trigger only
    default_args=default_args,
    start_date=datetime(2025, 3, 9),
    catchup=False,
    tags=['etl', 'vinculum', 'orderpull', 'sequential'],
)
...
extract_task = TrackedPythonOperator(
    ...
    pipeline_name='vinculum_order_pull',
    client_id='bsc',
...
trigger_next = TriggerDagRunOperator(
    ...
    trigger_dag_id='vinculum_order_pull_sequential',  # Self-trigger
