...
from connections.bsc.vinculum.invoicedetail.load import load_data
...
def get_next_date_range(**context):
    ...
    result = tracker.get_data_registry(
        client_id='bsc',
        data_type='vinculum-shipmentdetail',
        is_current='true',
        limit=1,
        sort_by='run_id',
        sort_order='DESC'
    ...
    # Get T-1 date (yesterday) for comparison
...
# Create DAG directly
dag = DAG(
    dag_id='vinculum_shipment_detail',
    description='ETL pipeline for Vinculum Shipment Detail data',
    schedule_interval='30 * * * *',  # Every hour at minute 30
    default_args=default_args,
    start_date=datetime(2025, 3, 9),
    catchup=False,
    tags=['etl', 'vinculum', 'shipmentdetail', 'shipment'],
)
