...
from connections._100days.enamor.shopify.orders.extract import extract_data
from connections._100days.enamor.shopify.orders.transform import transform_data_to_postgres
from connections._100days.enamor.shopify.orders.load import load_data
from plugins.operators.tracked_python_operator import TrackedPythonOperator

from connections._100days.enamor.soda_health_check.run_soda import run_scan as run_soda_scan

ALERT_EMAILS = [
    'swapnilmishra@bombayshavingcompany.com'
...
def get_all_date_ranges(**context):
    ...
    """
    pipelines = [
        ('enamor-shopify-orders', 'enamor-shopify-orders')
    ]
...
def create_extract_wrapper(extract_func, pipeline_name):
...
dag = DAG(
    dag_id='enamor_shopify_orders_pipeline',
    description='Enamor Shopify Order ETL',
    schedule_interval='30 3 * * *',  # Daily at 9 AM IST
    default_args=default_args,
    start_date=datetime(2026, 2, 1),
    catchup=False,
    max_active_runs=1,
    tags=['etl', 'shopify', 'enamor','100days'],
)
...
extract = TrackedPythonOperator(
    task_id='enamor_extract_from_source',
    python_callable=create_extract_wrapper(extract_data,"enamor-shopify-orders"),
    pipeline_name='enamor-shopify-orders',
    client_id='enamor',
    data_type='enamor-shopify-orders',
    is_first_task=True,
    is_last_task=False,
    dag=dag
)
...
transform = TrackedPythonOperator(
    task_id='enamor_transform_from_raw',
...
load = TrackedPythonOperator(
    task_id='enamor_load_to_iceberg',
...
def data_freshness_checks_warehouse():
    run_soda_scan(
        "freshness_raw_shopify_orders",
        "warehouse_db",
        ["checks/warehouse_db/raw_shopify_orders.yaml"]
    )
...
def run_shopify_orders_consolidated():
    ...
    query = """
        SELECT * FROM enamor.insert_shopify_consolidated();
    """
...
def run_shopify_sales_analysis():
    postgres = PostgresConnector(db_prefix="warehouse_")
    query = """
        CALL enamor.sp_shopify_sales_analysis();
    """
    postgres.execute_query(query)

consolidated_task = TrackedPythonOperator(
...
sales_analysis_task = TrackedPythonOperator(
...
complete = EmptyOperator(task_id='enamor_pipeline_complete',dag=dag)

# flow
start >> check_previous_run_task >> get_all_dates_task
get_all_dates_task >> extract >> transform >> load >> consolidated_task>> sales_analysis_task >> data_freshness_checks_audit_task >>data_freshness_checks_warehouse_task >> complete
