...
from connections.bsc.shopify.transaction.extract import extract_data
from connections.bsc.shopify.transaction.transform import transform_data
from connections.bsc.shopify.transaction.load import load_data
...
def get_all_date_ranges(**context):
    ...
    """
    pipelines = [
        ('bsc-shopify-transactions', 'bsc-shopify-transactions'),
        ('bombae-shopify-transactions', 'bombae-shopify-transactions'),
    ]
...
dag = DAG(
    dag_id='shopify_transactions_pipeline',
    description='Shopify Transactions ETL',
    schedule_interval='30 3 * * *',  # Daily at 9 AM IST
    default_args=default_args,
    start_date=datetime(2026, 2, 1),
    catchup=False,
    max_active_runs=1,
    tags=['etl', 'shopify', 'transactions'],
)
...
bombae_extract = TrackedPythonOperator(
    task_id='bombae_extract_from_source',
    python_callable=create_extract_wrapper(extract_data,"bombae-shopify-transactions","bombae"),
    pipeline_name='bombae-shopify-transactions',
    client_id='bsc',
    data_type='bombae-shopify-transactions',
    is_first_task=True,
    is_last_task=False,
    dag=dag
)

bombae_transform = TrackedPythonOperator(
    task_id='bombae_transform_from_raw',
    python_callable=transform_data,
    op_kwargs={"store": "bombae"},
    pipeline_name='bombae-shopify-transactions',
    client_id='bsc',
    data_type='bombae-shopify-transactions',
    is_first_task=False,
...
bombae_load = TrackedPythonOperator(
    task_id='bombae_load_to_iceberg',
    ...
    is_first_task=False,
    is_last_task=True,
    trigger_rule='all_done',  # Ensure this runs even if previous tasks failed
    dag=dag
)

bombae_complete = EmptyOperator(task_id='bombae_pipeline_complete',dag=dag)
...
extract_bsc = TrackedPythonOperator(
    task_id='bsc_extract_from_source',
    python_callable=create_extract_wrapper(extract_data,"bsc-shopify-transactions", "bsc"),
    pipeline_name='bsc-shopify-transactions',
    client_id='bsc',
    data_type='bsc-shopify-transactions',
    is_first_task=True,
    is_last_task=False,
    trigger_rule='all_done',  # Ensure this runs even if previous tasks failed
    dag=dag
...
# ---------------- TRANSFORM ----------------
transform_bsc= TrackedPythonOperator(
    task_id='bsc_transform_from_raw',
    python_callable=transform_data,
    op_kwargs={"store": "bsc"},
    pipeline_name='bsc-shopify-transactions',
    client_id='bsc',
...
# ---------------- LOAD ----------------
load_bsc= TrackedPythonOperator(
    task_id='bsc_load_to_iceberg',
    python_callable=load_data,
    op_kwargs={"store": "bsc"},
    pipeline_name='bsc-shopify-transactions',
    client_id='bsc',
...
get_all_dates_task >> bombae_extract >> bombae_transform >> bombae_load >> bombae_complete

# bsc flow
bombae_complete >> extract_bsc>> transform_bsc >> load_bsc>> end